...
  const p_ref = x.map(xi => xi <= L ? sourceTaper(xi) * reflection * Math.sin(omega * time + k * xi) : null);
  // 合成場
...
      <h1>声道音響インタラクティブ教材（音圧・粒子速度切替）</h1>
      <div className="card">
        <label>声道モデル</label>
...
        {/* 声道 */}
        ...
        {(showF1 || showF2) && (
          <g>
            {showF1 && <text x={0.01} y={-0.9} fontSize={0.15}>F1 ≈ {Math.round(F1)} Hz</text>}
            {showF2 && <text x={0.01} y={-0.6} fontSize={0.15}>F2 ≈ {Math.round(F2)} Hz</text>}
          </g>
        )}

        {x.map((xi, i) => field[i] !== null && (
          <circle key={i} cx={xi} cy={-field[i]} r={0.004} fill={mode === "pressure" ? "seagreen" : "orange"} />
        ))}
...
      </svg>
      ...
      <p className="legend">
        表示切替：音圧（腹＝大）／粒子速度（節＝大）<br />
        粒子速度は進行波−反射波の疑似表示（教育用）
      </p>
    </div>
  );
}
